<?php
 ...
 if (!isset($_SESSION["admin"])) {
  ...
  $statement = $conn->prepare("SELECT * FROM admins WHERE id = :id LIMIT 1");
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["username"])) {
      ...
      $statement = $conn->prepare("UPDATE admins SET username = :username WHERE id = :id");
      $statement->execute([
        ":id" => $id,
      ...
      $statement = $conn->prepare("UPDATE admins SET username = :username, picture = :destiny  WHERE id = :id");
      $statement->execute([
        ":id" => $id,
        ":username" => $_POST["username"],
        ":destiny" => $destiny,
      ...
      $_SESSION["flash"] = ["message" => "Admin {$_POST['username']} actualizado."];
        ...
        <div class="card-body">
          <?php if ($error): ?>
            <p class="text-danger">
              <?= $error ?>
            <?php endif ?>
          <form method="POST" enctype="multipart/form-data" action="edit_user.php?id=<?= $admins["id"] ?>">
            ...
            <div class="mb-3 row">
              ...
              <div class="col-md-6">
                <input value="<?= $admins["username"]?>" id="username" type="text" class="form-control" name="username" autocomplete="username" autofocus>
              </div>
          </div>
