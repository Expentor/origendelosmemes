  ...
  if (empty($_POST["username"]) || empty($_POST["email"]) || empty($_POST["password"]) || empty($_POST["confirm_password"])) {
    ...
    if ($statement->rowCount() > 0) {
      ...
      if ($picture == "image/png" || $picture == "image/jpeg" || $picture == "image/gif") {
        ...
        $conn
          ->prepare("INSERT INTO admins (username, email, password, picture) VALUES (:username, :email, :password, :destiny)")
          ->execute([
        ...
        <div class="card-body">
          <?php if ($error) : ?>
            <p class="text-danger">
              <?= $error ?>
            <?php endif ?>
            <form method="POST" action="register_admins.php" enctype="multipart/form-data">
              <div class="mb-3 row">
            ...
            <a href="loginAdmins.php">¿Ya tienes una cuenta?</a>
        </div>
      </div>
    </div>
  </div>
